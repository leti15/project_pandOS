...
{   //appena entra qui e' in kernel mode con interrupt disabilitati!!!
    ...
    if (exCode == 0){ interrupt_handler(); } //External Device Interrupt
    else if(exCode >=1 && exCode <= 3){ PassUpOrDie(PGFAULTEXCEPT); } //eccezioni TLB
    else if((exCode > 3 && exCode < 8) || (exCode > 8 && exCode < 13)){ PassUpOrDie(GENERALEXCEPT);} //program trap exception handler
    else if(exCode == 8){ SYS_handler(); } //system calls
    else{
...
void SYS_handler()
{
    if( STATUSC_GET_MODE(current_proc->p_s.status) == 1) { sys_terminate(); } //il processo era in user mode!
    else{
        ...
        {
            sys1();
            if (current_proc->p_s.reg_a1 == NULL){  sys_terminate(); } 
            
            pcb_PTR newProc = allocPcb(); //creo la stuttura per il nuovo processo
            
            if (newProc == NULL){ current_proc->p_s.gpr[1] = -1; }//non ci sono pcb liberi= mancanza risorse
        ...
        else if(current_a0 == 2) //terminate process SYSCALL(TERMPROCESS, 0, 0, 0);
       {
        ...
        else if(current_a0 == 5) //wait for IO    SYSCALL(IOWAIT, intlNo, dnum, termRead);
        {
            ...
            */
            sys5();
            if (current_proc->p_s.reg_a1 == NULL || current_proc->p_s.reg_a2 == NULL){ sys_terminate(); }

            int intlNo = current_proc->p_s.reg_a1, dnum = current_proc->p_s.reg_a2;
            int dev_pos = (intlNo - 3)*8 + dnum;
 
            if (current_proc->p_s.reg_a3 == 1){ dev_pos = dev_pos + 8; }//è un terminale in lettura = READ
            ...
            if (device[dev_pos] == NULL )
            {
                ...
                if (check_dev_installation(intlNo, dnum) == 1)
                {
                    ...
                    if( newsem == TRUE){ PANIC();} // se la insert blocked restituisce TRUE qualcosa è andato storto
                    else{ device[dev_pos] = newsem; } //aggiorno array di semafori dei device
        ...
        {   //ora c'è una risorsa disponibile quindi sblocco un processo
            ...
            { 
                ...
                {   
                    ...
                    if (headBlocked(temp) == NULL){ device[find_dev_index(temp)] = NULL; }
